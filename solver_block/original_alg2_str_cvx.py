...
def original_alg2_str_cvx(x, proximal_x, proximal_r, A, Ablk, Atblk, index, if_average, niter, **kwargs):
    ...
    for k in range(niter):
        ...
        partial_x = Atblk[k % blk_x].dot(yy)

        # update x_tilde
        mask = index[k % blk_x]
        x_old = x_tilde[mask]
        x_tilde[mask] = x_new = proximal_x(x_old - sigma * partial_x, sigma)

        # update x
        diff_Ax_tilde = Ablk[k % blk_x].dot(x_new - x_old)
        x[mask] = x[mask] + (tau / tau0) * (x_new - x_old)
        ...
        # update tau rho eta
        tau = (tau/2) * ((tau**2 + 4)**0.5 - tau) #c_tau * tau0 / (k + c_tau)
        rho = rho / (1 - tau) #rho0 * tau0 / tau
        eta = rho / c_eta
