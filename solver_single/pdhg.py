...
def pdhg(x, proximal_x, proximal_y, A, tau, sigma, if_average, niter, **kwargs):
    ...
    A : matrix in R^m*n
    tau : scalar, step size for primal variable x.
    sigma : scalar, step size for dual variable y.
    ...
    *Stochastic Primal-Dual Hybrid Gradient Algorithm with Arbitrary Sampling
    ...
    for k in range(niter):
        ...
        # temp = y_old + sigma * (A*x)
        temp = y_old + sigma * (A.dot(x))

        # update y
        y = proximal_y(temp, sigma)
