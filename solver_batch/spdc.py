...
def spdc(x, proximal_x, proximal_y, A, tau, sigma, theta, if_average, niter, **kwargs):
    ...
    for k in range(niter):
        ...
        for i in selected:
            ...
            temp = y_old + sigma[i]*(A[i,:].dot(x_bar))

            # update y
            y[i] = proximal_y(i, temp, sigma[i])
            
            # update dz
            dz += At[:,i].dot(np.array([y[i]-y_old]))
    ...
    return {"primal_opt": xx, "dual_opt": y, \
