...
def spdhg(x, proximal_x, proximal_y, A, tau, sigma, if_average, niter, **kwargs):
    ...
    extra: list. List of local extrapolation paramters for every index i. By default extra_i = 1.
    fun_select : Function that selects blocks at every iteration. By
    ...
    *Stochastic Primal-Dual Hybrid Gradient Algorithm with Arbitrary Sampling
    ...
    for k in range(niter):
        ...
        for i in selected:
            ...
            temp = y_old + sigma * (A[i,].dot(x))

            # update y
            y[i] = y_new = proximal_y(i, temp, sigma)

            # update dz
            dz += extra * At[:, i].dot(y_new - y_old)
        ...
        if verbose:
            if k % print_dist == 0:
                print(
                    '{:^14.4f}'.format(int(k*batch_size/m)), '|',
