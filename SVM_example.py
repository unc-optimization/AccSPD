...
By default the whole dimension is separated as 32 blocks. If you want to test the performance under different blocks:

    change the "num_blk" variable in this script.
...
if data_name == "w8a":
    sigma = 0.005
if_average = 1
Sol["pdhg"] = pdhg(x0, proximal_x, proximal_r_star, A, tau, sigma, if_average, int(niter / blk_r),
                   primal_obj_func=primal_obj_func, dual_obj_func=dual_obj_func,
                   verbose=1, print_dist=int(niter / blk_r / 20))
"""
Run SPDHG algorithm
"""
...
Sol["spdhg"] = spdhg(x0, proximal_x, proximal_r_star, A, Ablk_r, Atblk_r, index_r, tau, sigma, if_average, niter,
                     primal_obj_func=primal_obj_func, dual_obj_func=dual_obj_func,
                     theta=theta, verbose=1, print_dist=500)
